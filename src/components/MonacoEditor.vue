<template>
    <div id="monaco-box">
        <button @click="formatclick">格式化</button>
        <button @click="zipclick">压缩</button>
        <button @click="foldclick">折叠</button>
        <button @click="unfoldclick">展开</button>
...
export default {
    ...
    methods: {
        ...
        unfoldclick: function () {
            this.editor.trigger('anyString', 'editor.unfoldAll');
        },
        zipclick: function () {
            var newval = JSON.stringify(JSON.parse(this.editor.getValue()));
            this.editor.setValue(newval);
            // this.editor.trigger('anyString', 'editor.foldAll');
        },
        initEditor() {
            this.editor = monaco.editor.create(this.$refs.editor, {
                value: '{"key":111,"key2":222}',
                language: 'json',
                formatOnType: true,
                ...
                theme: 'vs'// vs, hc-black, or 
            })
            this.editor.onDidChangeModelContent((event) => {
                console.log(this.editor.getValue());
                // console.log(this.editor.getActions().map((item) => item.id));
                this.editor.getSupportedActions().forEach((value) => {
                    console.log(value);
                });
...
#monaco-editor {
    width: 100%;
    height: 600px;
    border: 3px solid rgba(12, 1, 1, 0.08);
}
